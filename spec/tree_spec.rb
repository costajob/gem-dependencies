...
describe Lapidarius::Tree do
  ...
  it "must collect dependencies" do
    tree.out.size.must_equal 6
  end

  it "must prepend dependencies with proper prefix" do
    tree.out[0].must_equal Stubs::Gems::sinatra.header
    ...
    tree.out[4].start_with?(Lapidarius::Tree::NESTED).must_equal true
    tree.out[5].start_with?(Lapidarius::Tree::CURVED).must_equal true
  end
end
